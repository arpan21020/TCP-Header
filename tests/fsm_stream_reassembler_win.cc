    ...
    try {
        ...
        for (unsigned rep_no = 0; rep_no < NREPS; ++rep_no) {
            ...
            if (buf.stream_out().bytes_written() != offset) {  // read bytes
                cout<<"offset:"<<offset<<" buf.stream_out().bytes_written():"<<buf.stream_out().bytes_written()<<endl;
                throw runtime_error("test 2.1 - number of RX bytes is incorrect");
            }
            if (!equal(result.cbegin(), result.cend(), d.cbegin())) {
                throw runtime_error("test 2.2 - content of RX bytes is incorrect");
            }
        }
